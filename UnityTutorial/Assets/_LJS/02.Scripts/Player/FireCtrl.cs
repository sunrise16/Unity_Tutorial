...
public class FireCtrl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(firePos.position, firePos.forward, out hit, 20.0f, 1 << layerMask))
        {
    ...
    {
        isReloading = true;
        _audio.PlayOneShot(playerSfx.reload[(int)curWeapon], 1.0f);

        // 재장전 오디오의 길이 + 0.3초 동안 대기
        yield return new WaitForSeconds(playerSfx.reload[(int)curWeapon].length + 0.3f);
